...
Public Class dataTiketForm
        ...
        Try
            If MessageBox.Show("Anda yakin ingin membatalkan " & dgv1.SelectedRows.Count & " data tiket ?", "Konfirmasi hapus", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                For Each ro As DataGridViewRow In dgv1.SelectedRows
                    Perintah = New MySqlCommand("DELETE FROM tiket WHERE id = '" & ro.Cells("id_tiket").Value & "'", Koneksi)
    ...
    Private Sub ToolStripButton4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripButton4.Click
        If sender.Text = "Cari" Then
            ...
            Dim searchq As String = " WHERE penjualan_tiket_view.id_tiket LIKE '%" & k & "%' OR penjualan_tiket_view.tanggal LIKE '%" & k & "%' OR penjualan_tiket_view.Hari LIKE '%" & k & "%' OR penjualan_tiket_view.Tujuan LIKE '%" & k & "%' OR penjualan_tiket_view.Mobil LIKE '%" & k & "%' OR penjualan_tiket_view.Supir LIKE '%" & k & "%' OR penjualan_tiket_view.NomorPolisi LIKE '%" & k & "%'"
